...
for n=1:n_max
    ...
    if(phi_wave<=0)
        ...
        while phi_wave>e_tol
            delta_lamda=phi_wave/(G+eta*eta_bar*K+xi*xi*H);
            J2_0=0.5*transpose(stress_correct)*P*stress_correct;
            stress_correct=stress_correct-delta_lamda*De*(0.5*P*stress_correct/sqrt(J2_0)+eta_bar*m);
            phi_wave=get_residual(delta_lamda);
        end       
