...
stress(:,1)=stress_0;
global strain_p_trial stress_trial;
...
for n=1:n_max
    ...
    if(phi_wave<=0)
        ...
        stress_i=stress(:,n);
        J2_0=0.5*transpose(stress_i)*P*stress_i;
        delta_lamda=(G*transpose(stress_i)*P/sqrt(J2_0)+eta*transpose(m)*De)*delta_eps/(G+eta*eta_bar*K+xi*xi*H);     
        delta_sigma=De*(delta_eps-delta_lamda*(0.5*P*stress_i/sqrt(J2_0)+eta_bar*m));
        strain_p(n+1)=strain_p(n)+delta_lamda;
        stress_1=stress_i+delta_sigma;
        stress(:,n+1)=stress_1;      
        sqrt_J2(n+1)=sqrt(0.5*transpose(stress_1)*P*stress_1);
    end
end
...
function phi_wave=get_residual(delta_lamda)
    global P eta m xi stress_trial;
    phi_wave=sqrt(0.5*transpose(stress_trial)*P*stress_trial)+eta*transpose(stress_trial)*m-xi*get_c(delta_lamda);
end
