clc;clear;
%Define constants
global G K H c0 eta eta_bar xi delta_gamma a lt2 lb2;
G=7.69e6; 
v=0.3;
Gc=G/2;
K=2*G*(1+v)/(3-6*v);
...
curvature=zeros(3,3,n_max);
curvature_e=zeros(3,3,n_max);
curvature_e0=zeros(3,3);
...
for n=1:n_max
    ...
    curvature(:,:,n+1)=curvature(:,:,n)+delta_cur;
    epsilon_e_trial=epsilon_e(:,:,n)+delta_eps;
    epsilon_e_dev_trial=get_dev_tensor(epsilon_e_trial);
    epsilon_e_vol_trial=trace(epsilon_e_trial);
    curvature_e_trial=curvature_e(:,:,n)+delta_cur;
    epsilon_p_bar_trial=epsilon_p_bar(n);
    s_trial=s(:,:,1)+G*(epsilon_e_dev_trial)+G*transpose(epsilon_e_dev_trial)+Gc*(epsilon_e_trial-transpose(epsilon_e_trial));
    p_trial=p(1)+K*epsilon_e_vol_trial;
    couple_trial=couple(:,:,1)+2*G*lb*lb*curvature_e_trial+(lt*lt-lb*lb)*eye(3);
    phi_wave=get_residual(s_trial,p_trial,couple_trial,epsilon_p_bar_trial);
    if phi_wave<=0
        s(:,:,n+1)=s_trial;
        p(n+1)=p_trial;
        stress(:,:,n+1)=s_trial+p_trial*eye(3);
        couple(:,:,n+1)=couple_trial;
        sqrt_J2(n+1)=get_J2(s_trial,couple_trial);
        epsilon_e(:,:,n+1)=epsilon_e_trial;
        epsilon_p_bar(n+1)=epsilon_p_bar_trial;   
        curvature_e(:,:,n+1)=curvature_e_trial;
    else
        while 1
...
function phi_wave=get_residual(s,p,cpl,e_p_bar)
...
function J2=get_J2(s,cpl)
    global a lt2 lb2;
    J2=a(1)*double_dot(s,s)+a(2)*double_dot(s,transpose(s))+a(3)*lb2*double_dot(cpl,cpl);
    J2=J2+a(3)*(1/lt2-1/lb2)*(cpl(1,1)*cpl(1,1)+cpl(2,2)*cpl(2,2)+cpl(3,3)*cpl(3,3));
end
