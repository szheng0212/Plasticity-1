...
for n=1:n_max
    ...
    if(phi_wave<=0)
        ...
        stress_dev=(1-G*delta_lamda/sqrt(J2_0))*(stress_trial-3*(transpose(stress_trial)*m)*m);
        stress_mean=transpose(stress_trial)*m-delta_lamda*eta_bar*K;
        stress_correct=stress_dev+3*stress_mean*m;           
